...
function PlatformsPage() {
  ...
  const fetchPlatforms = () => {
    setLoading(true);
    fetch('http://localhost:5055/api/Platforms')
      .then(res => res.json())
      .then(data => {
        setPlatforms(data);
        setLoading(false);
  ...
  const handleDelete = (id: number) => {
    fetch(`http://localhost:5055/api/Platforms/${id}`, { method: 'DELETE' })
      .then(res => {
        if (res.ok || res.status === 404) {
          fetchPlatforms();
        } else {
          alert("Cannot delete platform. It might be tied to a game.");
        }
      })
  ...
  return (
    <Box sx={{ maxWidth: '900px', margin: '0 auto', padding: '20px' }}>
      ...
      <Paper sx={{ padding: '20px', marginBottom: '30px', display: 'flex', gap: '15px', alignItems: 'center', backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '10px' }}>
        <TextField
          label="Platform Name (E.g., PlayStation 5, PC, Xbox)"
          variant="outlined"
          size="small"
          fullWidth
          sx={{
      ...
      {loading ? (
        <Typography sx={{ color: '#94a3b8', fontSize: '1.1rem' }}>Loading platforms...</Typography>
      ) : (
          ...
          <Table>
            <TableHead sx={{ backgroundColor: '#0f172a' }}>
              <TableRow>
                ...
                <TableCell sx={{ color: '#38bdf8', fontWeight: 'bold', fontSize: '0.95rem', borderBottom: '1px solid #334155' }}>Platform Name</TableCell>
                <TableCell align="right" sx={{ color: '#38bdf8', fontWeight: 'bold', fontSize: '0.95rem', borderBottom: '1px solid #334155' }}>Actions</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {platforms.map((plat) => (
                <TableRow key={plat.id} sx={{ '&:hover': { backgroundColor: '#334155' }, transition: 'background-color 0.15s' }}>
                  <TableCell sx={{ color: '#94a3b8', borderBottom: '1px solid #334155' }}>#{plat.id}</TableCell>
                  <TableCell sx={{ fontWeight: '700', color: '#ffffff', fontSize: '1.05rem', borderBottom: '1px solid #334155' }}>{plat.name}</TableCell>
                  <TableCell align="right" sx={{ borderBottom: '1px solid #334155' }}>
                    <Button variant="outlined" color="error" size="small" onClick={() => handleDelete(plat.id)} sx={{ fontWeight: '600' }}>
                      Delete
                    </Button>
                  </TableCell>
                </TableRow>
